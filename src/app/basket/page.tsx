...
import { useAuthUser } from '@/lib/auth' // ✅ 获取当前用户及其邀请码
...
export default function BasketPage() {
  ...
  useEffect(() => {
    const fetchBasket = async () => {
      ...
      const inviteCode = userDoc.exists() ? userDoc.data().inviteCode : null
      ...
      // 🟢 获取所有 recipeIds（遍历所有 7 天 × 3 餐）
      const recipeIds = weeklyMenu.flatMap((day: any) =>
        ['早餐', '午餐', '晚餐'].flatMap(meal =>
          Array.isArray(day[meal]) ? day[meal] : []
        )
      )
